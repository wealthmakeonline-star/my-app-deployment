...
const BusinessResults = ({ analysisResult, formData, onReset }) => {
  ...
  const INITIAL_RULE_STATE = {
    ...
    custom_pattern: '',
    operator_type: '',
    is_case_sensitive: 'n',  // Keep for form state
    rule_logic: ''
  };
  ...
  const handleSaveRule = async (e) => {
    ...
    try {
      // Match backend expected schema - IMPORTANT FIX
      const payload = {
        ...
        is_active: 1,
        created_by: 'web_user',
        // Backend expects 'case_sensitive' not 'is_case_sensitive'
